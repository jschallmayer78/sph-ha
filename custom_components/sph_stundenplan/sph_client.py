...
class SphClient:
    ...
    def __init__(self, school_id, username, password):
        ...
        self.session.headers["User-Agent"] = "Home Assistant SPH Stundenplan/0.2.1"
        self.key = None
    ...
    def _handshake(self):
        ...
        r = self.session.post(f"{SPH_BASE}/ajax.php", params={"f":"rsaPublicKey"}, timeout=15)
        r.raise_for_status()
        ...
        r = self.session.post(f"{SPH_BASE}/ajax.php", params={"f":"rsaHandshake","s":random.randrange(2000)}, data={"key":base64.b64encode(encrypted).decode()}, timeout=15)
        r.raise_for_status()
    ...
    def get_timetable(self):
        ...
        if all_table is None and own_table is None:
            raise RuntimeError("Kein Stundenplan für dieses Konto verfügbar.")
