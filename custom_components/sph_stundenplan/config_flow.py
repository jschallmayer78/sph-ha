...
from .const import DOMAIN, CONF_SCHOOL_ID, CONF_CLASS, CONF_UPDATE_INTERVAL, DEFAULT_UPDATE_INTERVAL
...
class SphConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        if user_input is not None:
            ...
            return self.async_create_entry(title=f"SPH {user_input[CONF_CLASS]}", data=user_input)
    ...
    def _schema(self, values=None):
        ...
        return vol.Schema({
            ...
            vol.Required(CONF_CLASS, default=values.get(CONF_CLASS, "")): str,
